...
const GraphCanvas: React.FC<GraphCanvasProps> = ({ nodes, edges, activeNodeId }) => {
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[500px] bg-slate-950 rounded-xl border border-slate-800 overflow-hidden shadow-inner">
      {/* Grid Background */}
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
        <defs>
          ...
          <marker id="arrowhead-active" markerWidth="10" markerHeight="7" refX="14" refY="3.5" orient="auto">
          ...
          return (
            <g key={edge.id}>
              ...
              {edge.label && (
                ...
                >
                   <span className="bg-slate-900 text-[10px] text-slate-400 px-1 rounded border border-slate-700 whitespace-nowrap transform -translate-x-1/2 shadow-sm">
                     {edge.label}
                   </span>
                </foreignObject>
              )}
      ...
      {nodes.map(node => {
        ...
        return (
          <motion.div
            key={node.id}
            className={`absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 w-28 cursor-pointer transition-all duration-300`}
          ...
          >
            {/* Node Shape */}
            <div className={`
              w-14 h-14 flex items-center justify-center rounded-xl border-2 shadow-xl backdrop-blur-sm transition-all duration-300
              ${getNodeColor(node)}
              ${isActive ? 'ring-4 ring-opacity-50 scale-105' : ''}
            `}>
              <Icon name={node.icon || 'Circle'} size={28} />
            </div>
            
            {/* Labels */}
            <div className="mt-2 flex flex-col items-center text-center min-w-[120px]">
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-slate-900/90 border border-slate-700 transition-colors ${isActive ? 'text-white border-white/30' : 'text-slate-300'}`}>
                {node.label}
              </span>
              {node.subLabel && (
                <span className="text-[10px] text-slate-500 mt-0.5 font-medium bg-slate-950/50 px-1 rounded">{node.subLabel}</span>
              )}
            </div>
          </motion.div>
        );
